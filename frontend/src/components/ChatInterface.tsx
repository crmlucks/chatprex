        ...
        <div className="flex-1 flex flex-col animate-in fade-in duration-200">
         ...
         {showReplyForm ? (
          <div className="flex-1 flex flex-col p-8 overflow-y-auto custom-scrollbar space-y-6 animate-in fade-in duration-300 max-w-2xl mx-auto w-full">
           ...
           <div className="space-y-2">
            <label className="label-text ml-1">Archivo adjunto (imagen, video, PDF)</label>
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
           <div className="p-5 border-b bg-surface-inset/50 dark:bg-surface-raised/30">
            <div className="relative group max-w-2xl mx-auto">
             ...
             <input type="text" placeholder="Buscar respuesta..." className={`w-full pl-10 pr-4 py-3 text-xs font-bold border rounded-2xl outline-none transition-all ${dc ? 'bg-surface-raised border-edge text-content focus:border-accent' : 'bg-surface border-edge focus:border-accent shadow-sm'}`} />
            </div>
           </div>
              ...
              quickReplies.map(reply => (
               <div key={reply.id} className={`relative p-4 rounded-xl border transition-all group ${dc ? 'bg-[#252525] border-edge hover:border-accent/50' : 'bg-surface border-edge-light hover:border-accent/30 shadow-sm hover:shadow-lg'}`}>
                <div className="flex justify-between items-start mb-2">
                 <h4 className={`text-sm font-bold group-hover:text-accent transition-colors ${dc ? 'text-content' : 'text-content'}`}>{reply.title}</h4>
