        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
          <div className="flex items-center gap-4">
             ...
             <div>
                ...
                <p className="body-text text-xs uppercase tracking-[2px] font-bold opacity-60">Difusión inteligente y masiva</p>
             </div>
          </div>
          ...
          <div className="col-span-1 lg:col-span-2 space-y-10">
            ...
            <div className="card-premium overflow-hidden">
              <div className={`px-8 py-6 border-b transition-all ${dc ? 'border-slate-800 bg-slate-900/30' : 'border-slate-50 bg-slate-50/30'}`}>
                ...
                <p className="body-text text-[10px] uppercase tracking-widest font-bold opacity-60">Personaliza tu alcance y estrategia</p>
              </div>
              <div className="p-8">
            ...
            <div className="card-premium overflow-hidden">
              <div className={`px-8 py-6 border-b flex items-center justify-between transition-all ${dc ? 'border-slate-800 bg-slate-900/30' : 'border-slate-50 bg-slate-50/30'}`}>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <button onClick={saveTemplate} className="text-[10px] font-black uppercase tracking-widest text-primary hover:bg-primary/10 px-3 py-1.5 rounded-lg transition-all">Guardar plantilla</button>
                </div>
              </div>
              <div className="p-8">
                <textarea
                  ...
                  className={`w-full h-40 p-6 rounded-3xl text-sm font-medium outline-none transition-all resize-none ${dc ? 'bg-slate-900/50 border border-slate-800 text-white focus:border-primary' : 'bg-slate-50 border border-slate-100 text-slate-800 focus:border-primary focus:bg-white shadow-inner'}`}
                />
                
                <div className="flex flex-wrap items-center gap-2 mt-6">
                  {['{{nombre}}', '{{proyecto}}'].map(v => (
                    <button key={v} onClick={() => setMessageTemplate(p => p + ' ' + v)} className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${dc ? 'bg-slate-800 text-slate-400 hover:text-white' : 'bg-white border border-slate-100 text-slate-500 hover:bg-slate-50 shadow-sm'}`}>+ {v}</button>
                  ))}
                ...
                {mediaName && (
                  <div className="mt-6 flex items-center gap-3 p-4 bg-emerald-500/10 text-emerald-500 rounded-2xl border border-emerald-500/20 shadow-inner">
                    <Paperclip size={18} />
                    <span className="flex-1 text-[11px] font-bold truncate">{mediaName}</span>
                ...
                {!mediaName && (
                  <div className="mt-6">
                    ...
                    <label htmlFor="campaign-media" className={`flex items-center justify-center gap-3 w-full py-5 border-2 border-dashed rounded-[24px] text-[11px] font-black uppercase tracking-widest transition-all cursor-pointer ${dc ? 'border-slate-800 text-slate-600 hover:border-primary hover:text-primary' : 'border-slate-100 text-slate-400 hover:border-primary hover:text-primary hover:bg-slate-50'}`}>
               ...
               <div className="p-8">
                  {recipientSource === 'manual' ? (
                    <textarea
                      ...
                      className={`w-full h-32 p-5 rounded-2xl text-xs font-mono outline-none transition-all ${dc ? 'bg-slate-900 text-white border border-slate-800' : 'bg-slate-50 border border-slate-100 text-slate-800 shadow-inner'}`}
                    />
                  ) : (
                    <div className="space-y-6">
                       <select 
                         ...
                         className={`w-full p-4 rounded-2xl text-sm font-bold outline-none border transition-all ${dc ? 'bg-slate-800 border-slate-700 text-white' : 'bg-slate-50 border-slate-100 text-slate-700 shadow-sm'}`}
                       >
                       ...
                       <div className={`p-6 rounded-[24px] flex items-center justify-between transition-all ${dc ? 'bg-primary/5 border border-primary/10' : 'bg-primary/5 border border-primary/10 shadow-sm'}`}>
                          <div className="flex items-center gap-4">
                             <div className="w-12 h-12 rounded-2xl bg-primary/10 flex items-center justify-center text-primary shadow-inner"><Users size={24} /></div>
                             <div>
                                <p className="text-[10px] font-black uppercase tracking-widest text-slate-500">Contactos seleccionados</p>
                                <p className="text-[11px] font-bold text-slate-400">Según el filtro actual del CRM</p>
                             </div>
                          </div>
          ...
          <div className="col-span-1 space-y-10">
            ...
            <div className="card-premium overflow-hidden flex flex-col h-[500px]">
              ...
              <div className="flex-1 bg-[#e5ddd5] p-6 relative overflow-y-auto" style={{ backgroundImage: 'url("https://user-images.githubusercontent.com/15075759/28719144-86dc0f70-73b1-11e7-911d-60d70fcded21.png")', backgroundSize: 'contain' }}>
                <div className="bg-[#d9fdd3] rounded-2xl p-4 shadow-xl max-w-[95%] ml-auto animate-in zoom-in-95 duration-300 border border-black/5">
            ...
            <div className="card-premium p-8">
               ...
               <div className="space-y-6">
                  <div>
                    ...
                    <div className="flex items-center gap-4">
                       <input type="number" value={minDelay} onChange={e => setMinDelay(Number(e.target.value))} className={`w-full p-4 rounded-2xl text-center font-black outline-none border transition-all focus:border-primary ${dc ? 'bg-slate-900 text-white border-slate-800' : 'bg-slate-50 text-slate-800 border-slate-100'}`} />
                       <span className="text-slate-300 font-black">—</span>
                       <input type="number" value={maxDelay} onChange={e => setMaxDelay(Number(e.target.value))} className={`w-full p-4 rounded-2xl text-center font-black outline-none border transition-all focus:border-primary ${dc ? 'bg-slate-900 text-white border-slate-800' : 'bg-slate-50 text-slate-800 border-slate-100'}`} />
                    </div>
                  </div>
                  <div className={`p-4 rounded-2xl flex items-start gap-3 shadow-inner ${dc ? 'bg-emerald-500/5 text-emerald-400' : 'bg-emerald-50 text-emerald-700 border border-emerald-100'}`}>
                     <AlertCircle size={18} className="shrink-0 mt-0.5" />
                     <p className="text-[10px] font-bold leading-tight uppercase tracking-wider opacity-80">Recomendamos 15-45 seg para envíos mayores a 50 contactos.</p>
                  </div>
               </div>
            </div>
            ...
            {/* Progreso */}
            {status !== 'idle' && (
              <div className="card-premium p-8 border-primary/20 animate-in slide-in-from-bottom duration-500">
                ...
                <div className="w-full bg-slate-100 dark:bg-slate-800 h-3 rounded-full overflow-hidden mb-6 shadow-inner">
                ...
                <div className="grid grid-cols-2 gap-4">
                   <div className="text-center p-4 rounded-2xl bg-emerald-500/5 shadow-inner">
                   ...
                   <div className="text-center p-4 rounded-2xl bg-rose-500/5 shadow-inner">
...
const StrategyCard = ({ icon, title, desc, active, onClick, isDarkMode }: any) => (
  <button 
    onClick={onClick}
    className={`p-6 rounded-[28px] border text-left transition-all active:scale-[0.98] ${
      active 
        ? 'border-primary bg-primary/5 shadow-xl shadow-primary/10' 
        : (isDarkMode ? 'border-slate-800 bg-slate-900/30 hover:border-slate-700' : 'border-slate-100 bg-white hover:border-slate-200 shadow-sm')
    }`}
  >
    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-4 transition-all shadow-md ${active ? 'bg-primary text-white shadow-lg shadow-primary/20' : (isDarkMode ? 'bg-slate-800 text-slate-400' : 'bg-slate-50 text-slate-500')}`}>
      {icon}
    </div>
    <h3 className={`text-[13px] font-bold mb-1 tracking-tight ${active ? (isDarkMode ? 'text-white' : 'text-slate-900') : (isDarkMode ? 'text-slate-300' : 'text-slate-700')}`}>{title}</h3>
    <p className={`text-[10px] font-medium leading-tight ${active ? 'text-primary' : 'text-slate-500'}`}>{desc}</p>
  </button>
);
