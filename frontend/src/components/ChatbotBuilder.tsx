...
const ChatbotBuilder = ({ isDarkMode }: { isDarkMode?: boolean }) => {
 ...
 const dc = isDarkMode;
 const inputCls = `w-full border rounded-xl px-4 py-3 text-sm font-bold transition-all focus:outline-none focus:ring-4 focus:ring-accent/10 ${dc ? 'bg-surface-raised border-edge text-content focus:border-accent' : 'bg-surface border-edge text-content focus:border-accent shadow-sm'}`;
 const labelCls = `block text-xs font-bold text-content-muted mb-2 ml-1`;
 const cardCls = `flex items-center justify-between p-5 border rounded-2xl transition-all ${dc ? 'bg-surface-raised/40 border-edge hover:bg-surface-raised/60' : 'bg-surface border-edge hover:shadow-md'}`;

 if (loading) {
  return (
   <div className={`flex-1 flex flex-col items-center justify-center ${dc ? 'bg-surface-base' : 'bg-surface-base'}`}>
    <div className="relative">
      <div className="w-16 h-16 border-4 border-accent/20 border-t-primary rounded-full animate-spin"></div>
      <Bot className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-accent" size={24} />
    </div>
    <p className="mt-4 text-xs font-bold uppercase tracking-normal text-content-muted animate-pulse">Iniciando asistente...</p>
   </div>
  );
 }
  ...
  <div className={`flex-1 overflow-y-auto p-4 md:p-8 pb-24 md:pb-8 flex flex-col gap-8 transition-colors ${dc ? 'bg-surface-base' : 'bg-surface-base'}`}>
   ...
   <div className={`w-full lg:w-80 shrink-0 card-premium flex flex-col overflow-hidden`}>
    <div className={`p-6 border-b flex justify-between items-center transition-colors ${dc ? 'bg-surface-raised/50 border-edge' : 'bg-surface-inset border-edge-light'}`}>
     ...
     <span className={`text-xs font-bold px-2 py-0.5 rounded-lg ${dc ? 'bg-surface-raised text-content-muted' : 'bg-slate-100 text-content-muted'}`}>{bots.length}</span>
    </div>
    <div className="flex-1 overflow-y-auto p-3 space-y-2 custom-scrollbar">
     {bots.map(b => (
      <button key={b.id} onClick={() => setSelectedBotId(b.id)}
       className={`w-full text-left px-4 py-4 rounded-2xl flex items-center gap-4 transition-all active:scale-[0.98] group ${selectedBotId === b.id ? 'bg-accent text-content shadow-sm shadow-accent/20' : (dc ? 'hover:bg-surface-raised text-content-secondary' : 'hover:bg-surface-inset text-content-secondary shadow-sm border border-edge-light hover:border-edge')}`}>
       <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 transition-colors ${selectedBotId === b.id ? 'bg-surface/20 text-content' : (dc ? 'bg-surface-raised text-accent' : 'bg-slate-100 text-accent')}`}>
         <Bot size={20} />
       </div>
       <div className="flex-1 overflow-hidden">
        <p className={`text-sm font-bold truncate ${selectedBotId === b.id ? 'text-content' : (dc ? 'text-content' : 'text-content')}`}>{b.name}</p>
        <p className={`text-xs font-medium truncate opacity-70`}>{b.provider} • {b.model}</p>
       </div>
      </button>
     ))}
    </div>
    <div className={`p-4 border-t transition-colors ${dc ? 'border-edge' : 'border-edge'}`}>
     <button onClick={() => {
     ...
     }} className="w-full py-3 bg-surface-raised dark:bg-slate-700 text-content text-xs font-bold rounded-xl hover:opacity-90 transition-all shadow-lg active:scale-95">
      + Nuevo asistente
     </button>
    </div>
   </div>
   ...
   <div className="flex-1 card-premium flex flex-col transition-all overflow-hidden">
     ...
     <div className="flex-1 w-full">
      <div className="flex items-center gap-4 mb-2">
       ...
       <input type="text" value={botName} onChange={e => setBotName(e.target.value)}
        className={`text-2xl font-bold bg-transparent border-none focus:ring-0 w-full transition-all tracking-tight ${dc ? 'text-content' : 'text-content'}`} />
      </div>
      <p className="body-text mt-1">Configuración y personalidad del asistente virtual</p>
     </div>
     <div className="flex gap-3 shrink-0">
       <span className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold shadow-sm ${hasApiKey ? 'bg-emerald-500/10 text-emerald-500 border border-emerald-500/20' : 'bg-amber-500/10 text-amber-500 border border-amber-500/20'}`}>
    ...
    {/* Tab Navigation */}
    <div className={`flex px-8 border-b overflow-x-auto custom-scrollbar transition-colors ${dc ? 'bg-surface border-edge' : 'bg-surface border-edge'}`}>
     {[
      ...
      <button key={t.id} onClick={() => setActiveTab(t.id)}
       className={`py-5 px-6 text-xs font-bold transition-all relative whitespace-nowrap flex items-center gap-2 ${activeTab === t.id ? 'text-accent' : 'text-content-muted hover:text-content-secondary'}`}>
       {t.icon} {t.label}
       {activeTab === t.id && <div className="absolute bottom-0 left-0 right-0 h-1 bg-accent rounded-t-full"></div>}
      </button>
     ))}
    </div>
     ...
     {activeTab === 'prompt' && (
      <div className="space-y-8 animate-in slide-in-from-right duration-300">
       <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
        ...
        <button onClick={() => setPrompt(DEFAULT_PROMPT)} className="text-xs font-bold text-accent flex items-center gap-2 bg-accent/5 px-4 py-2 rounded-xl border border-accent/10 hover:bg-accent/10 transition-all">
       ...
       <textarea
        ...
        className={`w-full h-80 p-8 rounded-xl border text-sm font-medium focus:ring-4 focus:ring-accent/10 outline-none transition-all resize-none leading-relaxed ${dc ? 'bg-surface-raised/50 border-edge text-content-secondary focus:border-accent' : 'bg-surface-inset border-edge text-content-secondary focus:border-accent '}`}
      ...
      <div className="space-y-10 animate-in slide-in-from-right duration-300">
       ...
       <div className="space-y-2">
        ...
        <div className="flex justify-between items-center px-1">
          ...
          <button onClick={testConnection} className="text-xs font-bold text-accent hover:underline underline-offset-4 transition-all">Probar conexión ahora</button>
        </div>
       </div>

       <div className={`p-8 rounded-xl border flex flex-col items-center text-center gap-4 transition-all ${dc ? 'bg-surface-raised/20 border-edge' : 'bg-surface-inset/50 border-edge'}`}>
      ...
      <div className="space-y-8 animate-in slide-in-from-right duration-300">
        ...
        <div className="space-y-2">
         ...
         <textarea
          ...
          className={`w-full h-80 p-8 rounded-xl border text-sm font-medium outline-none transition-all resize-none leading-relaxed ${dc ? 'bg-surface-raised/50 border-edge text-content-secondary focus:border-accent' : 'bg-surface-inset border-edge text-content-secondary focus:border-accent '}`}
         />
        </div>
      </div>
     )}
       ...
       {[
        ...
        <div key={i} className={cardCls}>
         <div className="flex items-center gap-5 pr-4">
          <div className={`w-11 h-11 rounded-xl flex items-center justify-center shrink-0 ${dc ? 'bg-slate-700 text-content-secondary' : 'bg-slate-100 text-content-muted'}`}>
            {item.icon}
          </div>
          <div>
           <h4 className={`text-sm font-bold ${dc ? 'text-content' : 'text-content'}`}>{item.label}</h4>
         ...
         <label className="relative inline-flex items-center cursor-pointer shrink-0">
          ...
          <div className="w-12 h-6 bg-slate-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-surface after:border-slate-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-accent dark:bg-surface-raised dark:after:border-slate-600"></div>
         </label>
        </div>
       ))}
       <div className={`${cardCls} flex-col items-start gap-4`}>
         <div className="w-full">
          ...
          <input 
           ...
           className={`w-full p-4 rounded-2xl border text-sm font-medium outline-none transition-all ${dc ? 'bg-surface-raised border-edge text-content focus:border-accent' : 'bg-surface-inset border-edge text-content focus:border-accent '}`}
          />
         </div>
       </div>
      </div>
    ...
    {/* Editor Footer */}
    <div className={`p-8 border-t flex flex-col sm:flex-row items-center justify-between gap-6 transition-colors ${dc ? 'bg-surface-raised/50 border-edge' : 'bg-surface-inset border-edge-light'}`}>
     <div className="flex items-center gap-3">
       {saveMsg && (
        <div className={`flex items-center gap-3 px-5 py-2.5 rounded-xl text-xs font-bold animate-in fade-in slide-in-from-left duration-300 ${saveMsg.includes('✅') ? 'bg-emerald-500/10 text-emerald-500' : 'bg-rose-500/10 text-rose-500'}`}>
   ...
   <div className={`w-full rounded-xl border shadow-sm flex flex-col overflow-hidden transition-all ${dc ? 'bg-surface border-edge' : 'bg-surface border-edge'}`}>
    <div className={`p-6 border-b flex justify-between items-center transition-colors ${dc ? 'bg-surface-raised' : 'bg-surface-raised'}`}>
     <div className="flex items-center gap-3">
      ...
      <div>
       <p className="text-xs font-bold text-content uppercase tracking-normal">Simulador en vivo</p>
       <p className="text-xs font-medium text-content-muted">Entorno seguro de pruebas</p>
      </div>
     </div>
     <button onClick={() => setSimMessages([])} className="text-content-muted hover:text-content transition-all active:rotate-180 duration-500"><RefreshCw size={18} /></button>
    </div>
    ...
    <div className={`flex-1 p-6 flex flex-col gap-5 overflow-y-auto custom-scrollbar min-h-[400px] transition-colors ${dc ? 'bg-[#0f0f0f]' : 'bg-[#efeae2]'}`}
     ...
     {simMessages.length === 0 && (
      <div className="flex-1 flex flex-col items-center justify-center text-center space-y-4 px-6 animate-in fade-in duration-1000">
       <div className="w-20 h-20 rounded-full bg-surface-raised/10 border-2 border-dashed border-edge-light0/20 flex items-center justify-center"><Bot size={32} className="text-content-muted opacity-20" /></div>
       <p className="text-xs font-bold text-content-muted uppercase tracking-normal">Inicie una prueba</p>
       <p className="body-text text-xs italic">Salude al bot para ver cómo interactúa con las reglas definidas.</p>
      </div>
     )}
     {simMessages.map((msg, i) => (
      <div key={i} className={`${msg.role === 'user' ? 'self-end' : 'self-start'} p-4 rounded-[22px] text-xs max-w-[85%] shadow-sm animate-in slide-in-from-bottom-2 duration-300 ${msg.role === 'user' ? 'bg-accent text-content rounded-tr-none' : (dc ? 'bg-surface-raised text-content rounded-tl-none border border-edge' : 'bg-surface text-content rounded-tl-none')}`}>
       <p className="font-medium leading-relaxed">{msg.text}</p>
       <span className={`text-xs block text-right mt-2 font-bold opacity-60`}>{msg.time}</span>
      </div>
     ))}
     {simLoading && (
      <div className={`self-start p-4 rounded-[22px] rounded-tl-none shadow-sm flex gap-1.5 ${dc ? 'bg-surface-raised border border-edge' : 'bg-surface'}`}>
    ...
    <div className={`p-5 border-t flex gap-4 transition-colors ${dc ? 'bg-surface border-edge' : 'bg-surface border-edge shadow-[0_-4px_12px_rgba(0,0,0,0.02)]'}`}>
     <div className={`flex-1 flex items-center px-5 py-3.5 rounded-2xl border transition-all ${dc ? 'bg-surface-raised border-edge focus-within:border-accent' : 'bg-surface-inset border-edge focus-within:border-accent focus-within:bg-surface'}`}>
      <input
       type="text"
       value={simInput}
     ...
     <button onClick={sendSimMessage} disabled={simLoading}
      className="w-12 h-12 rounded-2xl bg-accent text-content flex items-center justify-center shrink-0 hover:bg-accent-dark transition-all active:scale-90 shadow-lg shadow-accent/30 disabled:opacity-50">
      <Send size={20} />
     </button>
    </div>
   </div>
